...
import glob, os
from pjotrs_probeersel_helpers import crossover, mutation, get_children, fitfunc
import csv
import time
...
class evo_algorithm:
    ...
    def play_game(self, player, g, avg_gains, enemies, mapping):
        '''
        ...
        for enemy in enemies:
            ...
            contr = player[:self.n_vars]*mapping
    ...
    def simulate(self, pop = []):
        ...
        for g in range(self.generations):
            ...
            results = [pool.apply_async(self.play_game, args=(player, self.current_generation, avg_gains, enemy, mapping[k])) for k, player in enumerate(pop)]
            pop = []
            mapping = []
            ...
            for ind, result in enumerate(results):
                ...
                if fitness > self.max_gain:
                    ...
                    print(r[9])
                    self.best = r[0]
                    self.best[:265] = self.best[:265]*r[9]
                    print(self.best)
                    
                if survive:
                    surviving_players.append(ind)
            pool.close()
            avg_gains = 0.1*np.mean(fitness_array) + 0.9*np.max(fitness_array)
...
#                    avg_gains *= 0.9
            ...
            pop, mapping = get_children(pop, surviving_players, np.array(fitness_array),
                               mutation_baseline, mutation_multiplier, mapping)
            ...
            print(f'Run: {self.run}, G: {self.current_generation}, F_mean = {round(np.mean(fitness_array),1)} pm {round(np.std(fitness_array),1)}, F_best = {round(np.max(fitness_array),1)}, {np.sum(mapping[np.where(fitness_array == np.max(fitness_array))[0]])}, G_mean = {np.round(np.mean(gains_array),1)}, G_best = {np.round(np.max(gains_array))}, S_mean={mean_sigmas} max:{max_sigmas} min:{min_sigmas}, kills={np.round(np.max(np.sum(kills_array, axis=1)),1)}, surviving={len(surviving_players)}, thr={np.round(avg_gains, 1)} time={round(time.time()-gen_start)}')
        return
