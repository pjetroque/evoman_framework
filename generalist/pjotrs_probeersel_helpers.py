...
def get_children(parents, surviving_players, fitness, mutation_base, mutation_multiplier, mapping):
    children = copy.deepcopy(parents)
    children_mapping = copy.deepcopy(mapping)
    ...
    for i in range(len(parents)):
        ...
        if fitness[p1[i]] > fitness[p2[i]]:
            child_map = mapping[p1[i]]
        else:
            child_map = mapping[p2[i]]
        
        DNA   = child[:265]
        sigma = child[265:]

        #mutate based on parents fitness
        mutation_rate = 1-0.5*(fitness[p1[i]] + fitness[p2[i]])/(np.max(fitness)+0.1)
        if 0.5*(fitness[p1[i]] + fitness[p2[i]]) == np.max(fitness):
            print(mutation_rate)
        ...
        children[i] = child
        children_mapping[i] = child_map
    return children, children_mapping
